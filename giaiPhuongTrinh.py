...
factsName = ['a', 'b', 'c', 'gocA', 'gocB', 'gocC', 'ha', 'hb', 'hc', 'S', 'p', 'R', 'r', 'sinA', 'sinB', 'sinC',
             'cosA', 'cosB', 'cosC']
facts = [None] * 19
baseKnown = []
rules = []
...
def run():
    print('start')
    # doc file rules
    with open(r'rules.json') as json_file:
        data = json.load(json_file)
        for p in data['rules']:
            r = RuleClass(p['idx'], p['concepts'], p['expression'])
            rules.append(r)

    # code tam. doc gia thiet de bai
    with open(r'facts.json') as json_file:
        data = json.load(json_file)
        count = 0
        # doc gia thiet
        for p in data['facts']:
...
class RuleClass(object):
    def __init__(self, idx, concept_list, expression):
        self.idx = idx
